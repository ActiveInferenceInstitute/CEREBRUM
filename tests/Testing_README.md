...
python3 -m pytest tests/core/test_model.py
...
python3 tests/models/test_linear_regression.py
```
...
When you run the tests, various reports are generated in the `tests/output` directory:
...
`tests/output/all_YYYYMMDD_HHMMSS_coverage_html/index.html`

The main test report can be accessed at:
`tests/output/all_YYYYMMDD_HHMMSS_html/report.html` 